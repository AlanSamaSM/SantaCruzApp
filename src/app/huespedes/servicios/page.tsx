      ...
      <div className="mx-auto max-w-lg">
        <div className="mb-6">
          ...
          <p className="text-sm text-text-secondary">
            Servicios adicionales exclusivos de {PROPERTY_NAME}
          </p>
        </div>
              ...
              {services
                ...
                .map((service) => (
                  <button
                    ...
                    className="flex w-full items-center gap-3 rounded-xl border border-border bg-surface p-4 text-left shadow-sm transition-colors hover:bg-surface-secondary"
                  >
                    <span className="text-2xl">{service.icon}</span>
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-text-primary">{service.name}</p>
                      <p className="truncate text-xs text-text-secondary">{service.description}</p>
                    </div>
                    <div className="flex shrink-0 items-center gap-1">
                      <span className="text-sm font-bold text-brand-accent">
                        ${service.price.toLocaleString()}
                      </span>
                      <span className="text-[10px] text-text-secondary">{service.currency}</span>
                      <ChevronRight className="ml-1 h-4 w-4 text-text-secondary" />
                    </div>
                  </button>
                ))}
            </div>
          ...
          {selectedService && (
            <>
              ...
              <motion.div
                ...
                className="fixed inset-x-0 bottom-0 z-50 rounded-t-2xl border-t border-border bg-surface p-6 shadow-2xl pb-[calc(1.5rem+env(safe-area-inset-bottom))]"
              >
                ...
                <p className="mb-4 text-sm text-text-secondary">{selectedService.description}</p>
                ...
                {sent ? (
                  <div className="flex flex-col items-center gap-2 py-6">
                    ...
                    <p className="font-semibold text-text-primary">¡Solicitud enviada!</p>
                    <p className="text-xs text-text-secondary">Te confirmaremos pronto.</p>
                  </div>
                ) : (
                  <>
                    <div className="mb-3">
                      <label className="mb-1 flex items-center gap-1.5 text-xs font-medium text-text-secondary">
                        <Calendar className="h-3.5 w-3.5" /> Fecha deseada (opcional)
                      </label>
                      <input
                        type="date"
                        value={requestDate}
                    ...
                    <div className="mb-4">
                      <label className="mb-1 flex items-center gap-1.5 text-xs font-medium text-text-secondary">
                        <MessageSquare className="h-3.5 w-3.5" /> Comentarios (opcional)
                      </label>
                      <textarea
                        value={requestMessage}
                        onChange={(e) => setRequestMessage(e.target.value)}
                        rows={2}
                        placeholder="Ej: Necesito el transporte a las 8am"
                        className="w-full rounded-xl border border-border bg-surface-secondary px-4 py-3 text-sm outline-none focus:border-brand-accent"
                      />
                    </div>
                    <button
